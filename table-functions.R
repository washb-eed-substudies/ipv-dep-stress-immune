  ...
  if(adj_only){
    ...
    for (i in 1:length(exposure)) {
      for (j in 1:length(outcome)) {
        ...
        sigadj <- ifelse(filtered_adj$corrected.Pval < 0.2, "*", "")
    ...
    for (i in 1:length(exposure)) {
      for (j in 1:length(outcome)) {
        ...
        tbl <- rbind(tbl, list(s_name, e_name, out_var[j], filtered_adj$N[1], round(filtered_adj$Vlevel[1], 2),
                               v1, round(filtered_adj$Pval[1], 2), round(filtered_adj$corrected.Pval[1], 2), "", ""))
        tbl <- rbind(tbl, list(" ", " ", " ", " ", round(filtered_adj$Vlevel[2], 2), 
                               v2, round(filtered_adj$Pval[2], 2), round(filtered_adj$corrected.Pval[2], 2), 
  ...
  if(adj_only){
    ...
    for (i in 1:length(exposure)) {
      for (j in 1:length(outcome)) {
        ...
        if(j==1|skipped==T){
          tbl <- rbind(tbl, list(expo_var[i], out_var[j], filtered_res$N, round(filtered_res$q1, 2), round(filtered_res$q3, 2), 
                                 unadj, round(filtered_res$Pval, 2), round(filtered_res$corrected.Pval, 2), 
                                 adj, round(filtered_adj$Pval, 2), round(filtered_adj$corrected.Pval, 2)))
          skipped<-F
        }else {
          tbl <- rbind(tbl, list("", out_var[j],  filtered_res$N, round(filtered_res$q1, 2), round(filtered_res$q3, 2), 
                                 unadj, round(filtered_res$Pval, 2), round(filtered_res$corrected.Pval, 2), 
